...
33) Discord 交互偏好补丁 v1.3：在 Discord 中，凡涉及授权、选项选择、继续/暂停、验收级别、模式切换，默认优先使用可点击按钮/组件，而不是要求用户打字回复。高风险动作（删除/安装/推送/启用高自治实验模式）必须在按钮消息中明确写出动作内容；若当前上下文不适合按钮，则退回文字确认。
34) SYSTEM_BOOTSTRAP 维护策略：`SYSTEM_BOOTSTRAP.md` 为当前系统快速接管入口，不视为静态历史文档。凡默认工作流拓扑、路由/验收/模式系统、规则入口与优先级、关键常量（路径/频道/优先级/默认执行假设）发生稳定变化时，必须事件驱动更新；此外每 7–14 天做一次轻量一致性检查。更新时保持简短，并与 `rules/INDEX.md`、`skills/user-preferences/SKILL.md` 对齐。
35) 工作流模式持久化策略：所有模式（strict/proactive/evolver/ralph）默认均**仅限单条任务一次性**，任务完成后自动回退到 default 模式。除非用户明确指令“长期有效”且再次确认，否则不持续启动。即使长期有效，每次任务前仍需确认是否继续保持该模式（strict 除外）。
...
36) 智能交互提示方案 v1.4：在6类关键决策点自动弹出可点击按钮（复杂任务起始、高影响动作前、工作流模式选择、任务受阻时、验收决策点、定期巡检）。按钮颜色区分风险等级，内容必须先于按钮呈现，15分钟超时，打字回复可覆盖。若 Discord 组件不可用，回退到编号列表选择。
